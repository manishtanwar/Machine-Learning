...
void growNode(node *n,vi &rem_data,vi rem_attr,int depth, vi split_counter);

void produce_children(node *n,vi &rem_data,vi rem_attr,int depth, vi split_counter){
	double best_IG = 0.0;
	int best_attr = 30;
	...
	for(int i=0;i<child_cnt;i++){
		...
		if(is_best_cont){
			...
			growNode(n->child[i], rem_data_child[i], rem_attr_child, depth, split_counter);
			if(isSeparable) rem_attr_child.pop_back();
		}
		else growNode(n->child[i], rem_data_child[i], rem_attr_child, depth, split_counter);
	}
}
...
void growNode(node *n,vi &rem_data,vi rem_attr,int depth, vi split_counter){
	...
	if(n->y0 == 0 || n->y1 == 0 || rem_attr.size() == 0){
		n->leaf = 1;
		global_count_vec.push_back(split_counter);
		// -------------- debug ------------------
	...
	produce_children(n,rem_data,rem_attr,depth+1,split_counter);
}
...
void train_it(){
	...
	root = new node();
	vi split_counter(attr_cnt, 0);
	growNode(root,rem_data,rem_attr,0, split_counter);

	trace(split_counter);

	for(auto z : global_count_vec){
		for(int i =0;i<attr_cnt;i++)
			split_counter[i] = max(split_counter[i], z[i]);
	}
	int i =0;
	for(auto z : split_counter){
		i++;
		cout<<"X"<<i<<'\t'<<z<<endl;
		// trace(i, z);
	}
