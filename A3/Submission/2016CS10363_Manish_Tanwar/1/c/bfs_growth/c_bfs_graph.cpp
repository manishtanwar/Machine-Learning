...
void test_it_acc(vector<vi> &data, ofstream &fout, int train_hai){
	int correct_pred;
	correct_pred = 0;
	...
	for(auto &e : data){
		...
		while(1){
			...
			if(n->leaf || baccha == NULL || baccha->y == 0){
				if(n->y0 > n->y1) pred = 0;
				else pred = 1;
				break;
			}
	...
	if(train_hai)
		fout << node_cnt << "\t";
	fout << 100.0 * (double)correct_pred/data.size() << '\n';
}
...
void growNode(node *n,vi &rem_data,vi rem_attr){
	...
	produce_children(n,rem_data,rem_attr);
		...
		test_it_acc(train, fout_train, 1);
		test_it_acc(test, fout_test, 0);
		test_it_acc(val, fout_valid, 0);
	}
}
